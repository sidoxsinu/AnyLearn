...
**Solution:** A generated **concept graph** (prerequisite DAG) → modules → lessons → tasks → quizzes → resources, held as one versioned object and rendered as a React SVG roadmap canvas + lesson workspace.
**Why chat alone doesn't equal it:** A chat can *describe* a curriculum, but the curriculum isn't an object with state, links, and a native UI that stays consistent across sessions. Here, a quiz result, a resource and a lesson edit all reference the same concept IDs.
**Why learners care:** They see where they are, what's next, what's skipped and why. Unknown-unknowns are surfaced by the graph.
**Demo:** Enter goal → nodes spring onto the SVG canvas with prerequisites wired; tap a node → full lesson workspace loads. Say: "Nothing here was typed as a prompt after the first sentence."
...
**Demo:** Diagnostic (skips "Ohm's law"); then deliberately miss 2 questions on "ground planes" → roadmap canvas gains a remedial node that springs in, node turns amber, diff banner explains.
...
**Demo:** Report "I don't get what a ground plane is" as *missing prerequisite* → new prerequisite micro-lesson node springs into the canvas, downstream lesson edited to reference it, changelog entry, verifier ✅. Then tap **Undo** to show control.
...
**Demo:** Open a lesson → embedded YouTube player with channel credit and "why this fits: shows KiCad trace routing at the stage you're at"; tap a claim → source chip pops up; show a broken link auto-replaced (or pre-seeded to show the check).
